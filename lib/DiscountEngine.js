...
function loyaltyDiscount (total) {
  return nearestPenny(total * 0.98);
}

function generateDiscountFunction (customerHasLoyaltyCard) {
  return customerHasLoyaltyCard
    ? compose(loyaltyDiscount, bulkDiscount, sumItems)
    : compose(bulkDiscount, sumItems);
}

module.exports = {
  generateDiscountFunction: generateDiscountFunction,
